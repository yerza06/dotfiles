...
* **Всё уже написано по отдельности:** `NetworkPopup.qml`, `BluetoothPopup.qml`,
  `AudioPopup.qml`, `MprisPopup.qml` — задача сборочная, не с нуля.
...
### 3.8 Медиаплеер — ✅ сделано
* Реализовано в `status_bars/`: `MprisPopup.qml` (окно) и `MprisControlButton.qml`
  (круглая кнопка транспорта с состоянием «недоступна» и акцентной заливкой).
* Открывается по СКМ на модуле MPRIS: обложка из `trackArtUrl` с глифом-заглушкой,
  название, исполнитель, альбом, прогресс-бар с перемоткой и пять кнопок
  (перемешивание, предыдущий, play/pause, следующий, повтор). СКМ переехал
  с «предыдущего трека» — эта кнопка теперь в самом окне.
* Перемешивание (`shuffle`) и повтор (`loopState`) оформлены как в Spotify:
  включённый режим красится акцентом, повтор перебирается по кругу
  «выключен → плейлист → трек». Клавиши `Пробел`, `←`, `→`, `S`, `R`.
* Второе применение `PopupWindow` с `grabFocus` после календаря: окно «липкое»,
  закрывается кликом мимо, `Esc` или повторным СКМ.
* `position` у `MprisPlayer` вычисляется при чтении и сигнала сам не шлёт, поэтому
  `positionChanged()` эмитится таймером раз в секунду — только пока окно открыто
  и идёт воспроизведение.
* Первое применение `ClippingRectangle` из `Quickshell.Widgets` в `status_bars`
  (скругление обложки без `MultiEffect`).
* Очередь и переключение плееров не делались: выбор плеера остаётся в `MprisMenu`
  по ПКМ, очередь MPRIS-интерфейс `TrackList` в Quickshell не отдаёт.
...
### 3.11 Dock — ✅ сделано
* Реализован отдельным конфигом `dock/`: компактная автоскрывающаяся панель на каждом
  мониторе, закреплённые и запущенные приложения, группировка окон и контекстное меню.
* `DesktopEntries` отвечает за запуск и иконки, `ToplevelManager` — за живой список,
  экран, активацию и закрытие окон. Закрепления и порядок сохраняются между запусками.
* Оформление Floating Flexoki использует Flexoki Light/Dark, крупные иконки и мягкую
  тень средствами `MultiEffect`, не завися от blur Niri.

---

### 3.12 Окно Bluetooth — ✅ сделано
* Реализовано в `status_bars/`: `BluetoothPopup.qml` (окно), `BluetoothDeviceRow.qml`
  (строка устройства) и синглтон `BluetoothFormat.qml`. Открывается по ПКМ;
  ЛКМ по-прежнему уводит в `bluetui`, СКМ переключает адаптер.
* Шапка с именем адаптера и тумблером, «Видимость» и «Разрешить сопряжение»,
  списки `ПОДКЛЮЧЕНО` и `ДОСТУПНО`, глиф по типу устройства, заряд полосой,
  переключатель доверия и переименование под шевроном.
* Только сопряжённые устройства: сканирование, сопряжение и удаление намеренно
  оставлены `bluetui`, поэтому `adapter.discovering` окно не трогает.
* `BluetoothMenu.qml` удалён — его ПКМ забрало окно. Заодно повторён приём
  `NetworkFormat`: форматтеры уехали в синглтон, и виджет в панели больше
  не дёргает их у меню.
* Переименование пишет в `device.name` (`Alias` у BlueZ): `deviceName` только
  для чтения, и метка устройства теперь предпочитает `name` — иначе новое имя
  не было бы видно.
* MAC адаптера в окно не выведен: у `BluetoothAdapter` в Quickshell 0.3.1 есть
  только `name`, `adapterId` и `dbusPath`.
